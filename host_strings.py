...
class SubmitScript(FileBlock):
    def __init__(self):
        ...
        if domain=='prv.rosalind.compute.estate':
            ...
            'module load libs/lapack/3.8.0-gcc7.3.0 libs/openmpi/4.0.0-gcc7.3.0 libs/openblas/0.3.6-target-skylakex-gcc7.3.0' <<\
            'conda activate 2.7'

            #'module load {0}/gcc/7.4.0 {0}/openmpi/ib/3.1.4/gnu_7.4.0 {0}/openblas/0.3.6 {0}/lapack/3.8.0'.format(
    ...
    def render(self, fname, **kwargs):
        if 'partition' not in kwargs: kwargs['partition'] = '"morty,nms_research"'
        kwargs['driver_dir'] = os.path.abspath(__file__)
        kwargs['ncores'] = kwargs['nnodes']*kwargs['ntaskspernode']
        kwargs['files_to_node'] = ' '.join(kwargs['files_to_node'])
...
class Nevpt2Script(SubmitScript):
    def __init__(self, pops_dir=None, step_seed=False, loop=False):
        ...
        self << execution
    def render(self, fname, **kwargs):
        kwargs['files_to_node'] = ('FCIDUMP', '*.inp', 'nevpt2_store.pkl')
        SubmitScript.render(self, fname, **kwargs)

class Caspt2Script(FileBlock):
    def __init__(self):
        FileBlock.__init__(self)
        domain = get_domain()
        scheduler = schedulers[domain]
        ...
        if domain=='prv.rosalind.compute.estate':
            ...
            'module purge' <<\
            'export OMP_NUM_THREADS=32' <<\
            'module load {0}/gcc/7.4.0 {0}/boost/1.70/gcc7.4.0/python2.7'.format('/users/k1507071/opt/modules') <<\
            'module load libs/lapack/3.8.0-gcc7.3.0 libs/openmpi/4.0.0-gcc7.3.0 libs/openblas/0.3.6-target-skylakex-gcc7.3.0' <<\
            '. $HOME/intel/mkl/bin/mklvars.sh intel64'
            #'module load {} libs/lapack/3.8.0/gcc-7.3.0'.format('/opt/apps/etc/modules/libs/openmpi/4.0.0/gcc/7.3.0') <<\
        self << env_setup
        ...
        '    rsync $tmpdir/CORESPACE $WD' <<\
        '    rsync $tmpdir/CICJ_HISTOGRAM $WD' <<\
        '    rsync $tmpdir/caspt2.energy.dat $WD' <<\
        '    rsync $tmpdir/neci.out.* $WD' <<\
        '    rsync $tmpdir/RDMEstimates $WD' <<\
        '  fi' <<\
        '}}' <<\
        ...
        execution <<\
        'echo "initialising FCIQMC calculation"' <<\
        'mpirun -np {ncores} {neci_exe} initial_hbrdm.inp > neci.out.0' <<\
        'rm fciqmc_0_0.rdm*' <<\
        'ln -s spinfree_OneRDM.1 fciqmc_0_0.rdm1' <<\
        'ln -s spinfree_TwoRDM.1 fciqmc_0_0.rdm2' <<\
        'ln -s spinfree_ThreeRDM.1 fciqmc_0_0.rdm3' <<\
        'ln -s spinfree_CASPT2_AUX.1 fciqmc_0_0.rdm4f' <<\
        'smith_line=$(grep smith bagel.json)' <<\
        'export OMP_NUM_THREADS={ncores}'
        for exp in (5, 6):
            execution << r'sed s/"$smith_line"/"$smith_line \"thresh_overlap\": 1e-{},"/g bagel.json > tmp.json'.format(exp)
            execution << '{bagel_exe} tmp.json > bagel.out.0.'+str(exp)
        execution <<\
        'while true; do' <<\
        'iter=$(ls neci.out.* | wc -l)' <<\
        'mpirun -np {ncores} {neci_exe} continue_hbrdm.inp > neci.out.$iter' <<\
        'smith_line=$(grep smith bagel.json)'
        for exp in (3, 4, 5, 6):
            execution << r'sed s/"$smith_line"/"$smith_line \"thresh_overlap\": 1e-{},"/g bagel.json > tmp.json'.format(exp)
            execution << '{bagel_exe} tmp.json > bagel.out.$iter.'+str(exp)
        execution << 'done'
        self << execution
    def render(self, fname, **kwargs):
        ...
        kwargs['driver_dir'] = os.path.abspath(__file__)
        kwargs['ncores'] = kwargs['nnodes']*kwargs['ntaskspernode']
        kwargs['files_to_node'] = 'FCIDUMP *.inp FOCKMAT ref.archive bagel.json'
        FileBlock.render(self, fname, **kwargs)
